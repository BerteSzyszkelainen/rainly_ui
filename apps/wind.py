...
layout = html.Div(
    ...
    children=[
        ...
        html.Div(
            id="div-bar-chart-wind",
            children=dcc.Loading(children=dcc.Graph(id="bar-chart-wind"))
        ),
        html.Div(
            id="div-warning-wind",
...
@app.callback(
    Output(component_id='bar-chart-wind', component_property='figure'),
    Output(component_id='bar-chart-wind', component_property='style'),
...
def update_line_chart(day_count, n):
    ...
    if df.empty:
        ...
        fig.update_traces(hovertemplate="Data: %{x}<br>Średnia prędkość wiatru: %{y} km/h")
        fig.update_layout(
            hoverlabel=dict(
